...
[[ -z "$GIT_ENVIRONMENT_DEBUG" ]] && export GIT_ENVIRONMENT_DEBUG=0
[[ -z "$GIT_ENVIRONMENT_INTEGRATION" ]] && export GIT_ENVIRONMENT_INTEGRATION=1
[[ -z "$GITRC_ENVIRONMENT" ]] && export GITRC_ENVIRONMENT=0		# 0 = use the .private/repo_rc.sh file, 1 = use the $GIT_ROOT/.rc/rc file
...
#
# These script files are used along with the functions in this file to setup the
# environment for the git repository. Various scripts are used to control the
# user environment and behavior when entering/leaving a git repository.
#
#   GIT_PRIVATE_RC:
#       This is the path to the private repository resource file.
#       This is sourced when the git repository is changed to set any repository
#       specific environment variables.
#
#   BLD_TARGET_SCRIPT:
#       This is the path to the build target script.
#       This is sourced when the git repository is changed to set the $BLD_TARGET
#       environment variable.
#
#   NERDTREE_DEF_BOOKMARKS:
#       This is the path to the default NERDTree bookmarks file.
#       This is used when the NERDTree bookmarks file is not found or is out of
#       date to regenerate the bookmarks file.
#
#   NERDTREE_GEN_SCRIPT:
#       This is the path to the NERDTree bookmarks generator script.
#       This is used when the git repository is changed to generate the NERDTree
#       bookmarks file.
#
#   RETAG_SCRIPT:
#       This is the path to the retag script.
#       This is used when the git repository is changed to generate the ctags
#       files for the git repository.
#
export GIT_PRIVATE_RC=$HOME/.private/repo_rc.sh
export BLD_TARGET_SCRIPT=$HOME/.private/bld_target.sh
export NERDTREE_DEF_BOOKMARKS=$HOME/.private/NERDTreeDefaultBookmarks
export NERDTREE_GEN_SCRIPT=$HOME/sbin/gen_nerdtree_bookmarks.py
export RETAG_SCRIPT=$HOME/sbin/retag.sh

################################################################################
# update_external_environment()
#
# This function will update the external environment variables when the user changes directories.
#
...
#
# This function will source the resource files for the current repository. This is used to update any aliases
# that may have changed since the last time the resource files were sourced.
#
function source_resource_files() {
	local resource_files=(
		"$HOME/.private/aliases.sh"
		"$HOME/.aliases"
		"$GIT_ROOT/.rc/rc"
	)
...
# _print_git_env()
#
# This function will print the current git environment variables. This is used to debug the git environment.
#
...
# git_title_format()
#
# This function will format the git title information. This is used to display the git information in the terminal title.
#
...
# git_prompt_format()
#
# This function will format the git prompt information. This is used to display the git information in the prompt.
#
...
function initialize_git_repository() {
	...
	# If we are not using the GITRC_ENVIRONMENT, then just return
...
#
# This function will update the git environment variables. This is used to update the git environment variables when
